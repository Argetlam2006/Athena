...
def get_player_by_name(name: str, profile_type: ProfileType = ProfileType.CAREER) -> dict:
    ...
    matches = df[(df['normalized_name'].str.contains(name.lower())) & (df['profile_type'] == profile_type.value)]
    if matches.empty:
        return None
...
def run_player_capability_validation():
    ...
    validate_capability_ranking("chance_creation", [
        "lionel andrés messi",
        "mesut",
        "andrés iniesta",
        "xavi hernández"
    ...
    validate_capability_ranking("attacking_threat", [
        "lionel andrés messi",
        "thierry henry",
    ...
    validate_capability_ranking("defensive_activity", [
        "patrick vieira",
        "sergio busquets",
        "kolo touré"
    ])
