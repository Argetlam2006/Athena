...
from shared.schemas import RecruitmentCriteria, ProfileType
from tests.evaluation.player_capability_validation import get_player_by_name, get_full_profile
import pandas as pd
...
def validate_recruitment():
    ...
    if not busquets:
        ...
        if candidates:
            ...
            print(f"Top Candidate: {top_candidate.player.player_name} (Score: {top_candidate.fit_score:.1f})")
            
            p_cap = top_candidate.player.capability_profile
            if p_cap.ball_security.score >= 70.0:
                print(f"[PASS] Top candidate meets ball_security criteria (Score: {p_cap.ball_security.score:.1f})")
            else:
                print(f"[FAIL] Top candidate fails ball_security criteria (Score: {p_cap.ball_security.score:.1f})")
        else:
    ...
    if candidates:
        ...
        if p_cap.attacking_threat.score >= 80.0:
            print(f"[PASS] Top candidate meets attacking_threat criteria (Score: {p_cap.attacking_threat.score:.1f})")
        else:
            print(f"[FAIL] Top candidate fails attacking_threat criteria (Score: {p_cap.attacking_threat.score:.1f})")
    else:
        print("[FAIL] No candidates found.")
