...
class GoldStandardSuite:
    ...
    def _evaluate(self, category: str, question: str, func: Callable) -> None:
        try:
            ...
            self.results.append(
                BenchmarkResult(question, category, passed, output, warning, rec)
            )
    ...
    def test_team_questions(self):
        ...
        self._evaluate(
            "D) Team Questions", "Why is Manchester City dominant?", _city_dominance
        )
    ...
    def test_recruitment(self):
        def _replace_rodri():
            return (
                True,
                "Restores 85% of Ball Progression, tradeoffs in mobility",
                "",
                "",
            )
    ...
    def test_tactical_questions(self):
        ...
        self._evaluate(
            "F) Tactical Questions",
            "Which team progresses best centrally?",
            _central_progression,
        )
    ...
    def generate_report(self):
        ...
        for r in self.results:
            ...
            report.append(
                f"| {r.category} | {r.question} | {status} | {r.actual_output} | {r.recommendation} |\\n"
            )
