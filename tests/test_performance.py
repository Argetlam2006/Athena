...
def test_performance_budget_store_regen():
    ...
    assert duration < BUDGET_STORE_REGEN_SEC, (
        f"Store init took {duration:.2f}s, exceeding {BUDGET_STORE_REGEN_SEC}s budget"
    )
...
def test_performance_budget_player_lookup():
    ...
    assert duration < BUDGET_SIMILARITY_LOOKUP_SEC, (
        f"Player lookup took {duration:.2f}s, exceeding {BUDGET_SIMILARITY_LOOKUP_SEC}s budget"
    )
