...
from shared.schemas import ProfileType, CollectiveProfile
...
def test_recruitment_returns_non_empty_results():
    criteria = RecruitmentCriteria(
        ...
        required_capabilities={"ball_progression": 1.0}
    )
    candidates = search_candidates(criteria)
    assert len(candidates) > 0, "Recruitment returned no candidates for a generic search"

def test_team_grades_computed_dynamically():
    # Mock a team with average capability of 82.0 (Should be 'A')
    team = CollectiveProfile(
        team_id=1, team_name="Test", competition="Test", season="Test",
        avg_capabilities={
