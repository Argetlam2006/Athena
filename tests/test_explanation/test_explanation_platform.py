...
import pytest
from backend.explanation.prompt_builder import PromptBuilder, PromptPackage, SystemPromptBuilder
...
from backend.explanation.context import PlayerExplanationContext, EvidencePacket

def test_prompt_builder():
    ctx = PlayerExplanationContext(
        player_id=1, player_name="Test Player", team_name="Test Team",
        position_group="Forward", age_years=25, minutes_played=2000,
        archetype="Striker", overall_confidence=1.0,
        evidence_packets=[
            EvidencePacket(source="cap:1", title="Test Cap", confidence=1.0, supporting_metrics={"score": 90.0})
        ]
    )
...
def test_conversation_manager():
    ...
    changed = mgr.detect_and_handle_context_change(current_player_id=1, current_team_id=None, current_workspace="player_intelligence")
    ...
    changed2 = mgr.detect_and_handle_context_change(current_player_id=1, current_team_id=None, current_workspace="player_intelligence")
