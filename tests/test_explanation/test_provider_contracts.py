...
def test_all_providers_implement_generation_response_signature():
    ...
    for provider_cls in providers:
        ...
        assert gen_sig.return_annotation == GenerationResponse, f"{provider_cls.__name__}.generate must return GenerationResponse, got {gen_sig.return_annotation}"
        
        # Check stream signature
        stream_sig = inspect.signature(provider_cls.stream)
        annotation_str = str(stream_sig.return_annotation)
        assert "GenerationResponse" in annotation_str, f"{provider_cls.__name__}.stream must yield GenerationResponse, got {annotation_str}"
        assert "str" not in annotation_str.replace("GenerationResponse", ""), f"{provider_cls.__name__}.stream must not yield str"
