...
from shared.schemas import PlayerProfile, CapabilityProfile, CapabilityScore, TeamProfile
...
def valid_player_profile() -> PlayerProfile:
    cap = CapabilityProfile(
        player_id=1, player_name="Test Player", season="2023", competition="Ligue 1", position_group="Forward", minutes_played=2000,
        ball_progression=CapabilityScore("ball_progression", 85.0, 1.0, {"progressive_passes_p90": 5.0}),
        chance_creation=CapabilityScore("chance_creation", 98.0, 1.0, {"shot_assists_p90": 2.0}),
        ball_security=CapabilityScore("ball_security", 80.0, 1.0, {"pass_accuracy_pct": 85.0}),
    ...
    return PlayerProfile(
        player_id=1, player_name="Test Player", position_group="Forward", team_name="PSG", 
        competition="Ligue 1", season="2023", age_years=25, minutes_played=2000, 
        capability_profile=cap, decision_signals=["elite_goal_scorer"], archetype="Complete Forward"
    )
...
def test_valid_player_context_builds(valid_player_profile):
    ...
    bp_packet = next(p for p in ctx.evidence_packets if p.source == "capability:ball_progression")
...
def test_team_context_serialization():
    tp = TeamProfile(
        team_id=101, team_name="Test Team", competition="Ligue 1", season="2023",
        squad_size=25, avg_ball_progression=80.0, style_label="Direct"
    )
