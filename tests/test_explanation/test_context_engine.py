...
def valid_player_profile() -> PlayerProfile:
    cap = CapabilityProfile(
        ...
        ball_progression=CapabilityScore(
            ...
            85.0,
            1.0,
            [SupportingMetric("progressive_passes_p90", 5.0, 95.0, 0.5, "exp")],
        ),
        chance_creation=CapabilityScore(
            "chance_creation",
            98.0,
            1.0,
            [SupportingMetric("shot_assists_p90", 2.0, 98.0, 0.5, "exp")],
        ),
        ball_security=CapabilityScore(
            "ball_security",
            80.0,
            1.0,
            [SupportingMetric("pass_accuracy_pct", 85.0, 80.0, 0.5, "exp")],
        ),
        press_resistance=CapabilityScore("press_resistance", 90.0, 1.0, []),
        defensive_activity=CapabilityScore(
            "defensive_activity",
            80.0,
            0.9,
            [
                ...
                SupportingMetric(
                    ...
                    100.0,
                    0.0,
                    "Defensive Activity evaluated using Active philosophy.",
                ),
            ],
        ),
...
def test_valid_player_context_builds(valid_player_profile):
    ...
    assert any(
        m["metric_name"] == "score" and m["raw_value"] == 85.0
        for m in bp_packet.supporting_metrics
    )
    assert any(
        m["metric_name"] == "progressive_passes_p90" and m["raw_value"] == 5.0
        for m in bp_packet.supporting_metrics
    )
    ...
    assert any(
        m["metric_name"] == "defensive_philosophy"
        and m["explanation"] == "Defensive Activity evaluated using Active philosophy."
        for m in da_packet.supporting_metrics
    )
