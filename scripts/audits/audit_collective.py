...
def run_audit():
    ...
    def team_score(c: CollectiveProfile):
        return c.avg_capabilities.get("attacking_threat", 0) + c.avg_capabilities.get(
            "ball_security", 0
        )
    ...
    with open("collective_intelligence_report.md", "w") as f:
        ...
        f.write(
            "This report validates the deterministic extraction of Identity, Concentration, Fragility, and Bottlenecks from the Football Intelligence Engine.\n\n"
        )

        for team in top_teams:
            f.write(f"## {team.team_name}\n")
            f.write(
                f"**Primary Identity:** {team.identity.primary_identity if team.identity else 'Unknown'}\n"
            )
            ...
            if team.bottlenecks:
                for b in team.bottlenecks:
                    f.write(
                        f"- **{b.upstream_capability} ➔ {b.downstream_capability}**: {b.diagnosis} (Severity: {b.severity})\n"
                    )
            else:
            ...
            if team.fragility_map:
                top_frag = sorted(
                    team.fragility_map, key=lambda x: x.structural_deficit, reverse=True
                )[:3]
                for frag in top_frag:
                    f.write(
                        f"- **{frag.player_name}** (Deficit: {frag.structural_deficit})\n"
                    )
            ...
            for c in team.concentration:
                if c.is_over_centralized:
                    f.write(
                        f"- **WARNING: {c.capability_name.replace('_', ' ').title()} is over-centralized** (HHI: {c.hhi_score})\n"
                    )
