...
def select_model(models):
    ...
    flash_models = [m for m in models if "flash" in m.lower() and "lite" not in m.lower() and "preview" not in m.lower() and "image" not in m.lower() and "tts" not in m.lower() and "omni" not in m.lower()]
    if flash_models:
    ...
    pro_models = [m for m in models if "pro" in m.lower() and "preview" not in m.lower() and "image" not in m.lower() and "tts" not in m.lower() and "banana" not in m.lower() and "deep-research" not in m.lower()]
    if pro_models:
