...
from backend.retrieval.debug import is_debug_enabled
...
# Documented here for auditability.  Retrieved by name in retrieval_service.py.
...
class RetrievalPromptBridge:
    ...
    def build_prompt(
        ...
        prompt_pkg.metadata.update({
            ...
            "retrieval_plan_step_count": len(plan.steps) if retrieval_used else 0,
        ...
        if is_debug_enabled():
            prompt_pkg.metadata.update({
                "debug_traversal_summary": [
                    f"{step.step_type.value}:{step.edge_type.value if step.edge_type else ''}"
                    for step in plan.steps
                ],
                "debug_projected_claims": [claim.statement for claim in bundle.claims],
                "debug_evidence_bundle": {
                    "claim_count": len(bundle.claims),
                    "traversal_count": bundle.traversal_count,
                    "store_fingerprint": bundle.store_fingerprint,
                },
            })
