...
class OpenAIProvider(ExplanationProvider):
    def __init__(self, model_name: str = "gpt-4o-mini", temperature: float = 0.2, base_url: str | None = None, api_key: str | None = None):
        ...
        if model_name == "gpt-4o-mini" and not _is_valid(settings.OPENAI_API_KEY) and _is_valid(settings.NVIDIA_API_KEY):
        ...
        if not final_url and (self.model_name.startswith("meta/") or self.model_name.startswith("nvidia/")):
             if _is_valid(settings.NVIDIA_API_KEY):
                 final_key = settings.NVIDIA_API_KEY
                 final_url = "https://integrate.api.nvidia.com/v1"
        
        self.client = OpenAI(api_key=final_key, base_url=final_url) if HAS_OPENAI else None
    ...
    def is_available(self) -> bool:
        ...
        return HAS_OPENAI and (_is_valid(settings.OPENAI_API_KEY) or _is_valid(settings.NVIDIA_API_KEY))
