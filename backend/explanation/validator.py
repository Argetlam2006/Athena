...
def validate_evidence_packet(packet: EvidencePacket) -> None:
    ...
    if packet.confidence is None or not (0.0 <= packet.confidence <= 1.0):
        raise ContextValidationError(f"Invalid confidence value {packet.confidence} in packet {packet.title}.")
        
    if not packet.supporting_metrics:
        raise ContextValidationError(f"Missing supporting metrics in packet {packet.title}.")
        
    # If it's a capability packet, the score should ideally exist in supporting metrics.
    if packet.source.startswith("capability:"):
        cap_name = packet.source.split(":")[1]
        if "score" not in packet.supporting_metrics:
             raise ContextValidationError(f"Capability packet {packet.title} missing 'score' metric.")
...
def validate_player_context(ctx: PlayerExplanationContext) -> None:
    ...
    if not ctx.evidence_packets:
        raise ContextValidationError(f"Player context for {ctx.player_name} contains no evidence packets.")
        
    if ctx.overall_confidence is None or not (0.0 <= ctx.overall_confidence <= 1.0):
        raise ContextValidationError(f"Invalid overall confidence in player context for {ctx.player_name}.")
...
def validate_team_context(ctx: TeamExplanationContext) -> None:
    ...
    if ctx.squad_size <= 0:
        raise ContextValidationError(f"Team context for {ctx.team_name} has invalid squad size.")
        
    if not ctx.style_label:
        raise ContextValidationError(f"Team context for {ctx.team_name} missing tactical style label.")
...
def validate_recruitment_context(ctx: RecruitmentExplanationContext) -> None:
    ...
    if not ctx.candidates:
        raise ContextValidationError("Recruitment context has no candidates to explain.")
    ...
    for idx, candidate in enumerate(ctx.candidates):
        ...
        if "player_name" not in candidate:
            raise ContextValidationError(f"Candidate at index {idx} missing player_name.")
