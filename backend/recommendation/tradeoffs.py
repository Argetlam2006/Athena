...
def generate_tradeoffs(
    ...
    for cap_name, required_weight in criteria.required_capabilities.items():
        s = score(cap_name)
        ...
        if s >= 80:
            ...
            trade_offs.append(f"Below average {display_name} ({s:.1f}) relative to requirements")
    ...
    if score("physical_availability") < 50 and "physical_availability" not in criteria.required_capabilities:
        trade_offs.append(f"Availability concerns (Score: {score('physical_availability'):.1f})")
