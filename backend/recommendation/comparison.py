...
from shared.schemas import PlayerProfile, ComparisonResult
...
def compare_players(players: list[PlayerProfile]) -> ComparisonResult:
    ...
    if len(players) >= 2:
        ...
        for cap in capabilities:
            ...
            if max_score - min_score > 15:
                ...
                best_player = [name for name, s in cap_comparison[cap].items() if s == max_score][0]
                worst_player = [name for name, s in cap_comparison[cap].items() if s == min_score][0]
    ...
    if shared_strengths:
        summary += "They share elite output in " + ", ".join([s.replace("Strong ", "").lower() for s in shared_strengths]) + ". "
    if key_differences:
