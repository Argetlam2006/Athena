...
def evaluate_tactical_fit(
    ...
    if target_style == "Possession-Dominant":
        alignment = (
            (score("ball_security") * 0.50)
            + (score("ball_progression") * 0.25)
            + (score("press_resistance") * 0.25)
        )
    elif target_style == "High Press":
        alignment = (score("defensive_activity") * 0.60) + (
            score("press_resistance") * 0.40
        )
    elif target_style == "Direct and Progressive":
        alignment = (
            (score("ball_progression") * 0.50)
            + (score("chance_creation") * 0.30)
            + (score("attacking_threat") * 0.20)
        )
    elif target_style == "Counter-Attacking":
        alignment = (
            (score("attacking_threat") * 0.50)
            + (score("ball_progression") * 0.30)
            + (score("defensive_activity") * 0.20)
        )
    ...
    if team:
        ...
        weak_caps = (
            [c for c, v in team.avg_capabilities.items() if v < 60.0]
            if hasattr(team, "avg_capabilities")
            else []
        )
        if weak_caps:
            rel_sum = 0.0
    ...
    if (
        ...
        and player.player_attributes.tactical_versatility is not None
    ):
