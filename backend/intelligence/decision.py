...
class DecisionEngine:
    ...
    def explain_capability(
        profile: PlayerProfile, capability_name: str, cohort: list[PlayerProfile]
    ) -> CapabilityExplanation:
        """
        ...
        for metric in evidence:
            drivers[metric.metric_name] = (
                f"{int(round(metric.percentile))}th percentile"
            )

        return CapabilityExplanation(
            capability_name=capability_name, score=round(score, 1), drivers=drivers
        )

    @staticmethod
    def build_player_decision_card(
        profile: PlayerProfile, cohort: list[PlayerProfile]
    ) -> PlayerDecisionCard:
        """
        ...
        for cap_name in capabilities:
            cap_score_obj = getattr(
                profile.capability_profile, cap_name.lower().replace(" ", "_"), None
            )
            if cap_score_obj:
                score = cap_score_obj.score
                cohort_scores = []
                for p in cohort:
                    p_cap = getattr(
                        p.capability_profile, cap_name.lower().replace(" ", "_"), None
                    )
                    if p_cap:
                ...
                if cohort_scores:
                    ...
                    explanation = DecisionEngine.explain_capability(
                        profile, cap_name, cohort
                    )
                    if pct >= 85:
    ...
    @staticmethod
    def analyze_team_dependency(
        team: CollectiveProfile, squad: list[PlayerProfile]
    ) -> dict[str, DependencyAnalysis]:
        """
    ...
    @staticmethod
    def build_team_decision_card(
        team: CollectiveProfile,
        squad: list[PlayerProfile],
        cohort_teams: list[CollectiveProfile],
    ) -> TeamDecisionCard:
        """
        ...
        gap_analysis = {}
        capabilities = [
            "avg_ball_progression",
            "avg_chance_creation",
            "avg_ball_security",
            "avg_press_resistance",
            "avg_defensive_activity",
            "avg_attacking_threat",
        ]
        ...
        for cap in capabilities:
            ...
            cohort_scores = [
                getattr(t, cap, 0.0) for t in cohort_teams if getattr(t, cap, 0.0) > 0.0
            ]
                ...
                if team_score >= top_quartile:
                    ...
                    strengths.append(
                        CapabilityExplanation(
                            ...
                            score=team_score,
                            drivers={
                                "Advantage vs Top Quartile": f"+{round(team_score - top_quartile, 1)} points"
                            },
                        )
                    )
                    ...
                    weaknesses.append(
                        CapabilityExplanation(
                            ...
                            score=team_score,
                            drivers={
                                "Deficit vs Median": f"{round(team_score - median, 1)} points"
                            },
                        )
                    )
        ...
        return TeamDecisionCard(
            team=team,
            tactical_identity=team.identity.primary_identity
            if team.identity
            else "Balanced",
            biggest_strengths=sorted(strengths, key=lambda x: x.score, reverse=True)[
                :3
            ],
