...
def generate_collective_identity(
    players: list[PlayerProfile], global_pool: list[PlayerProfile] = None
) -> CollectiveIdentity:
    """
    ...
    secondary = (
        similarities[1][0]
        if len(similarities) > 1 and similarities[1][1] > 0.75
        else None
    )
    ...
    return CollectiveIdentity(
        primary_identity=primary, secondary_identity=secondary, emergent_traits=emergent
    )
