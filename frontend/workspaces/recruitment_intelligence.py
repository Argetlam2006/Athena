...
STYLE_MAP = {
    "Forward": [
        "Elite Goal Scorer", "Target Man", "Complete Forward", "Direct Winger",
        "Creative Forward", "Creative Playmaker", "Creative Winger"
    ],
    "Midfielder": [
        "Deep-Lying Playmaker", "Box-to-Box Engine", "Press-Resistant Anchor", "Midfield Destroyer",
        "Creative Playmaker" 
    ],
    "Defender": [
        "Ball-Playing Defender", "Progressive Fullback", "Traditional Defender", "Defensive Fullback"
    ],
...
def render() -> None:
    ...
    if st.session_state.parsed_query:
        if st.session_state.parsed_query.get("error"):
            st.error(f"Natural Language Parsing Failed: {st.session_state.parsed_query['error']}")
        else:
        ...
        with c1:
            st.markdown(f"**Position**<br/>✓ {pq.get('position', 'Any')}", unsafe_allow_html=True)
        with c2:
            st.markdown(f"**Style**<br/>✓ {pq.get('playing_style', 'Balanced')}", unsafe_allow_html=True)
        with c3:
            traits_str = "<br/>".join([f"✓ {t}" for t in pq.get('traits', [])]) if pq.get('traits') else "None specified"
            st.markdown(f"**Desired Traits**<br/>{traits_str}", unsafe_allow_html=True)
            
        st.markdown(f"<div style='margin-top: 1rem; font-size: 0.85rem; color: #9ca3af;'><strong>Interpretation Confidence:</strong> {pq.get('confidence', 'Medium')}</div>", unsafe_allow_html=True)
    ...
    with col_filters:
        ...
        if st.session_state.parsed_query and st.session_state.parsed_query.get("position"):
        ...
        with st.form("recruitment_form"):
            position = st.selectbox(
                "Choose Position", ["Forward", "Midfielder", "Defender", "Goalkeeper"],
                index=default_pos_idx
            )
            
            style_options = STYLE_MAP.get(position, ["Balanced"])
            default_style_idx = 0
            if st.session_state.parsed_query and st.session_state.parsed_query.get("playing_style"):
                style = st.session_state.parsed_query["playing_style"]
                if style in style_options:
                    default_style_idx = style_options.index(style)
                    
            playing_style = st.selectbox("Choose Playing Style", style_options, index=default_style_idx)

            st.markdown("<div style='margin-top: 1rem; font-size: 0.9rem; font-weight: 600;'>Must Have Traits</div>", unsafe_allow_html=True)
            
            parsed_traits = []
            if st.session_state.parsed_query and st.session_state.parsed_query.get("traits"):
    ...
    with col_results:
        ...
        if submitted:
            ...
            if w_bp > 0: required["ball_progression"] = w_bp
            if w_cc > 0: required["chance_creation"] = w_cc
            if w_at > 0: required["attacking_threat"] = w_at
            if w_da > 0: required["defensive_activity"] = w_da
            if w_bs > 0: required["ball_security"] = w_bs
            if w_pr > 0: required["press_resistance"] = w_pr
            ...
            if not candidates:
                ...
                for c in candidates:
                    ...
                    stars = "★★★★★ Excellent Fit" if c.fit_score > 85 else "★★★★☆ Strong Fit" if c.fit_score > 70 else "★★★☆☆ Good Fit"

                    with st.expander(
                        f"#{c.rank} • {c.player.player_name} • {stars}"
                    ):
                        ...
                        if c.player.capability_profile:
                            for cap_name, cap_obj in c.player.capability_profile.__dict__.items():
                                if cap_obj and getattr(cap_obj, 'score', 0) > 85:
                                    st.markdown(f"✓ Elite {cap_name.replace('_', ' ').title()}")
                                    has_strengths = True
                                elif cap_obj and getattr(cap_obj, 'score', 0) > 75:
                                    st.markdown(f"✓ Strong {cap_name.replace('_', ' ').title()}")
                                    has_strengths = True
