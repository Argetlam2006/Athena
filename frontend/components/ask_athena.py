...
def render_ask_athena_drawer() -> None:
    ...
    with col_badge1:
        st.markdown(f"<span style='background: #374151; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; color: #d1d5db;'>Provider: {provider.__class__.__name__}</span>", unsafe_allow_html=True)
    with col_badge2:
        st.markdown(f"<span style='background: #374151; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; color: #d1d5db;'>Model: {provider.model_name}</span>", unsafe_allow_html=True)
    with col_btn:
            ...
            with st.chat_message("assistant", avatar="◇"):
                ...
                if ctx_obj:
                    ...
                    try:
                        ...
                        # Telemetry
                        record_telemetry(ExplanationTelemetry(
                            provider=provider.__class__.__name__.replace("Provider", "").lower(),
                            model=provider.model_name,
                            prompt_version=prompt_pkg.prompt_version,
                            context_size_bytes=prompt_pkg.metadata["context_size_bytes"],
                    ...
                    st.info("I don't have enough structured context in this workspace to answer that yet.")
                    manager.add_assistant_message("I don't have enough structured context in this workspace to answer that yet.")
