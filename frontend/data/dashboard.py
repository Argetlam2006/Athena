...
def get_dashboard_summary() -> dict:
    ...
    career_profiles = (
        len(player_idx[player_idx["profile_type"] == ProfileType.CAREER])
        if not player_idx.empty
        else 0
    )
    season_profiles = (
        len(player_idx[player_idx["profile_type"] == ProfileType.SEASON])
        if not player_idx.empty
        else 0
    )
    competition_profiles = (
        len(player_idx[player_idx["profile_type"] == ProfileType.COMPETITION])
        if not player_idx.empty
        else 0
    )
    ...
    return {
        ...
        "total_leagues": player_idx[
            player_idx["profile_type"] == ProfileType.COMPETITION
        ]["competition"].nunique()
        if not player_idx.empty
        else 0,
