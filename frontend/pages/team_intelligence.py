...
from frontend.layout import render_page_header, render_section_header, render_divider, render_kpi_card
...
def render() -> None:
    ...
    if not state.selected_team_id:
        render_empty_state("◉", "No Team Selected", "Please select a focus team from the global context menu in the sidebar to view their intelligence profile.")
        return
        
    team = get_team_profile(state.selected_team_id)
    if not team:
        render_empty_state("⚠", "Team Not Found", "The selected team could not be found in the database.")
        return
    ...
    with col2:
        render_kpi_card("Avg Age", f"{team.average_age:.1f}" if team.average_age else "N/A")
    with col3:
    ...
    with col_style:
        ...
        st.info("Team-aggregated capability radar will be rendered here. (Awaiting squad aggregation layer)")
